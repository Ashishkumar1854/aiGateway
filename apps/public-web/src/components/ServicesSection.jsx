...
const services = [
  {
    ...
    color: 'border-[#1e1e2e] hover:border-orange-500/20',
  ...
  },
  {
    ...
    color: 'border-[#1e1e2e] hover:border-blue-500/20',
  ...
  },
  {
    ...
    color: 'border-[#1e1e2e] hover:border-pink-500/20',
  ...
  },
  {
    ...
    color: 'border-[#1e1e2e] hover:border-green-500/20',
    ...
    <section className="py-24 bg-[#08080f] border-b border-[#1e1e2e] relative overflow-hidden">
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 relative z-10">
        <div className="text-center mb-16">
          ...
          <h2 className="text-3xl font-extrabold text-white mt-5 sm:text-5xl tracking-tight">Automate with AI Assistants</h2>
          <p className="mt-4 text-slate-400 max-w-xl mx-auto text-sm leading-relaxed font-light">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {services.map((s) => (
            <div
              key={s.name}
              className={`rounded-2xl border bg-[#111118]/40 p-5 flex flex-col justify-between transition-all duration-300 hover:bg-[#111118]/80 hover:-translate-y-1 shadow-xl ${s.color}`}
            >
              <div>
                <span className={`text-2xl p-2.5 rounded-xl border inline-block ${s.iconColor}`}>
                  {s.icon}
                </span>
                <h3 className="mt-5 text-sm font-bold text-white">{s.name}</h3>
                <p className="mt-2 text-xs text-slate-400 leading-relaxed font-light">{s.desc}</p>
                <ul className="mt-5 space-y-2.5">
                  {s.features.map((f) => (
                    <li key={f} className="flex items-center gap-2 text-[11px] text-slate-350 font-light">
        ...
        <div className="mt-14 text-center">
          <Link
            href="/services"
            className="inline-flex rounded-xl bg-indigo-600 hover:bg-indigo-500 px-7 py-3.5 text-sm font-bold text-white transition-all shadow-lg shadow-indigo-600/10 hover:shadow-indigo-600/25 hover:-translate-y-0.5"
          >
